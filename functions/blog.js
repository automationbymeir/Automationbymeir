...
const SITE = "https://www.automationbymeir.com";
...
<head>
  ...
  <meta property="og:site_name" content="Automation by Meir">
  ...
  <style>
    ...
    .card { background: #26262b; border: 1px solid #35353c; border-radius: 14px; overflow: hidden; }
    ...
    article .content pre { background: #141416; border: 1px solid #35353c; padding: 16px; border-radius: 10px; overflow-x: auto; }
    ...
    article .content blockquote { border-inline-start: 3px solid #72FFA3; margin: 24px 0; padding: 4px 20px; color: #c9c9d1; }
    .faq { margin-top: 48px; }
...
const EMPTY_BODY = `<div class="blog-index"><h1>Blog</h1><p class="sub">Practical automation notes from Automation by Meir.</p><div class="empty"><p>First posts are on the way. Check back soon.</p></div></div>`;
...
async function renderIndex() {
  ...
  if (!data || data.__nokey || data.__error || !Array.isArray(data.data) || data.data.length === 0) {
    return page({
      title: "Blog | Automation by Meir",
      description: "Practical automation notes: Power Automate, Apps Script, AI workflows and integrations.",
      canonical: `${SITE}/blog`,
      body: EMPTY_BODY,
    });
  }
  ...
  return page({
    ...
    description: "Practical automation notes: Power Automate, Apps Script, AI workflows and integrations.",
    canonical: `${SITE}/blog`,
    body: `<div class="blog-index"><h1>Blog</h1><p class="sub">Practical automation notes: Power Automate, Apps Script, AI workflows and integrations.</p><div class="cards">${cards}</div></div>`,
  });
}
...
async function renderPost(slug) {
  ...
  const jsonLd = JSON.stringify({
    ...
    author: post.author ? { "@type": "Person", name: post.author.name } : { "@type": "Organization", name: "Automation by Meir" },
    image: hero || undefined,
    mainEntityOfPage: `${SITE}/blog/${post.slug}`,
  }).replace(/</g, "\\u003c");
  return page({
    title: post.metaTitle || `${post.title} | Automation by Meir`,
    description: post.metaDescription || post.description || "",
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      ...
      res.status(200).send(page({
        title: "Blog | Automation by Meir",
        description: "Practical automation notes from Automation by Meir.",
        canonical: `${SITE}/blog`,
        body: EMPTY_BODY,
      }));
    }
